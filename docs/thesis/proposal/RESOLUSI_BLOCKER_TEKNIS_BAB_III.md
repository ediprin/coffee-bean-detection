...
Paper Xu et al. mendukung urutan min-max recovered response, perkalian elemen dengan raw spatial domain, dan residual addition. Paper tidak menentukan aturan diskret DC maupun clipping pasca-residual. Karena itu kedua detail tersebut harus disebut sebagai kontrak transfer implementasi penelitian.
...
## 5. Precision/Recall dan Confidence pada Validasi
...
- `iou=0.7` merupakan default NMS dan `max_det` penelitian dioverride menjadi `500`;
...
Setelah verifikasi ini, naskah BAB III harus:

1. mengembalikan `C0` ke kontrak retained AF2 (DC → bin 0, residual tanpa clipping);
2. meniadakan status tentatif pada kontrak rentang output AF2;
