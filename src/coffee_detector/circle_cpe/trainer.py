...
def make_circle_cpe_trainer(
    config: CircleCPEConfig | dict[str, Any],
    *,
    d0_checkpoint: str | Path | None = None,
):
    from ultralytics.models.yolo.detect import DetectionTrainer

    frozen = CircleCPEConfig.from_mapping(config)
    bound_checkpoint = Path(d0_checkpoint).expanduser().resolve() if d0_checkpoint is not None else None

    class CircleCPEDetectionTrainer(DetectionTrainer):
        def get_model(self, cfg=None, weights=None, verbose=True):
            ...
            if transfer is not None:
                print(f"CIRCLE-CPE NATIVE HEAD TRANSFER: {transfer}", flush=True)
            return model
    ...
    CircleCPEDetectionTrainer.__name__ = "CircleCPEDetectionTrainer"
    return CircleCPEDetectionTrainer
