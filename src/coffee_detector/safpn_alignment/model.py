...
class SAFPNAlignmentConfig:
    ...
    @classmethod
    def from_mapping(cls, payload: "SAFPNAlignmentConfig | dict[str, Any] | None") -> "SAFPNAlignmentConfig":
...
def _pixel_base_grid(height: int, width: int, *, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
...
def _bilinear_align(feature: torch.Tensor, offset: torch.Tensor) -> torch.Tensor:
    ...
    if tuple(offset.shape[-2:]) != (height, width):
        raise ValueError("Resolusi offset harus sama dengan feature")
    ...
    y = 2.0 * y / float(height - 1) - 1.0 if height > 1 else torch.zeros_like(y)
    x = 2.0 * x / float(width - 1) - 1.0 if width > 1 else torch.zeros_like(x)
...
class SpatialAwareAlignmentFusion(nn.Module):
    def __init__(self, shallow_channels: int, deep_channels: int, *, offset_init_zero: bool = True) -> None:
        super().__init__()
    ...
    def forward(self, shallow: torch.Tensor, deep: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        pooled = torch.cat((joined.mean(dim=1, keepdim=True), joined.amax(dim=1, keepdim=True)), dim=1)
        weight = torch.sigmoid(self.spatial_weight(pooled))
        fused = weight * (
            _bilinear_align(deep_up, deep_offset) + _bilinear_align(shallow, shallow_offset)
        ) + deep_up + shallow
        return fused, {"weight": weight, "shallow_offset": shallow_offset, "deep_offset": deep_offset}


class SAFPNClassificationCorrection(nn.Module):
    def __init__(self, channels: tuple[int, int, int], num_classes: int, config: SAFPNAlignmentConfig) -> None:
        super().__init__()
        if len(channels) != 3:
            raise ValueError("SAFPN alignment memerlukan tepat P3/P4/P5")
        c3, c4, c5 = (int(value) for value in channels)
        self.config = config
        self.p5_to_p4 = SpatialAwareAlignmentFusion(c4, c5, offset_init_zero=config.offset_init_zero)
        self.p4_to_p3 = SpatialAwareAlignmentFusion(c3, c4, offset_init_zero=config.offset_init_zero)
        self.class_corrections = nn.ModuleList([nn.Conv2d(c3, num_classes, 1), nn.Conv2d(c4, num_classes, 1)])
...
class SAFPNAlignmentDetectHead(nn.Module):
    def __init__(self, base_head: nn.Module, config: SAFPNAlignmentConfig) -> None:
        ...
        if type(base_head).__name__ != "Detect" or not getattr(base_head, "end2end", False):
            raise TypeError("SAFPN alignment memerlukan native YOLO26 end-to-end Detect")
        channels = tuple(_first_conv_channels(branch) for branch in base_head.cv2)
        if len(channels) != 3:
            raise ValueError("SAFPN alignment memerlukan tiga level")
        ...
        for name in ("nc", "nl", "reg_max", "stride", "end2end", "max_det", "export", "format", "dynamic", "agnostic_nms"):
            if hasattr(base_head, name):
                setattr(self, name, getattr(base_head, name))
    ...
    def _forward_branch(self, features: list[torch.Tensor], branch: dict[str, nn.Module]) -> dict[str, torch.Tensor]:
        ...
        return {
            ...
            "scores": torch.cat([(logit + correction).view(batch, self.nc, -1) for logit, correction in zip(logits, corrections)], dim=-1),
            "feats": features,
        }
...
def inject_safpn_alignment(model: nn.Module, config: SAFPNAlignmentConfig | dict[str, Any] | None) -> int:
...
class SAFPNAlignmentDetectionModel(DetectionModel):
    def __init__(self, cfg="yolo26.yaml", ch=3, nc=None, verbose=True, safpn_alignment=None):
