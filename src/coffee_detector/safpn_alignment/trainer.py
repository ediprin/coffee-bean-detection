...
from .model import SAFPNAlignmentConfig, SAFPNAlignmentDetectionModel, load_safpn_alignment_detector_weights


def make_safpn_alignment_trainer(config: SAFPNAlignmentConfig | dict[str, Any], *, d0_checkpoint: str | Path | None = None):
    ...
    class SAFPNAlignmentTrainer(DetectionTrainer):
        def get_model(self, cfg=None, weights=None, verbose=True):
            model = self.set_model_names_for_load(SAFPNAlignmentDetectionModel(
            ...
            if bound is not None and not getattr(self.args, "resume", False):
                ...
                transfer = load_safpn_alignment_detector_weights(model, YOLO(str(bound)).model)
            elif weights:
