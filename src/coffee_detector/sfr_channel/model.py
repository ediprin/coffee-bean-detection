...
class LSHChannelFormer(nn.Module):
    def __init__(self, in_channels: int, config: SFRChannelConfig) -> None:
        ...
        self.mlp = nn.Sequential(nn.Linear(token_dim, hidden), nn.GELU(), nn.Linear(hidden, token_dim))
    ...
    def _partition(self, x: torch.Tensor):
        ...
        windows = x.view(b, c, hp // s, s, wp // s, s).permute(0, 2, 4, 3, 5, 1).reshape(-1, s * s, c)
    ...
    def _restore(self, windows, shape, batch):
        ...
        s = self.config.window_size
        c = windows.shape[-1]
        x = windows.view(batch, hp // s, wp // s, s, s, c).permute(0, 5, 1, 3, 2, 4).reshape(batch, c, hp, wp)
        return x[:, :, :h, :w]
    ...
    def forward(self, feature: torch.Tensor) -> torch.Tensor:
        ...
        channels = windows.transpose(1, 2).contiguous()
        ...
        gather = lambda t, o: torch.gather(t, 1, o.unsqueeze(-1).expand(-1, -1, t.shape[-1]))
        sorted_tokens = gather(channels, order)
        sorted_qk = gather(qk, order)
        sorted_v = gather(value, order)
        n, c, token_dim = sorted_qk.shape
        per_bucket = c // self.config.bucket_count
        q = sorted_qk.view(n, self.config.bucket_count, per_bucket, token_dim)
        v = sorted_v.view(n, self.config.bucket_count, per_bucket, token_dim)
        ...
        channels = gather(sorted_tokens, inverse)
...
class SFRChannelCorrection(nn.Module):
    def __init__(self, channels, num_classes, config):
        ...
        if config.mode == "spatial_channel":
            spatial_cfg = SFRSpatialConfig(hidden_dim=config.hidden_dim, num_heads=config.spatial_heads, window_size=config.window_size, mlp_ratio=config.mlp_ratio, correction_scale=config.correction_scale)
            self.spatial_blocks = nn.ModuleList([WindowSpatialFormer(c, spatial_cfg) for c in channels])
        else:
            self.spatial_blocks = None
        self.classifiers = nn.ModuleList([nn.Conv2d(config.hidden_dim, num_classes, 1) for _ in channels])
    ...
    def forward(self, features):
        output = []
        for index, (block, classifier, feature) in enumerate(zip(self.channel_blocks, self.classifiers, features)):
...
class SFRChannelDetectHead(nn.Module):
    ...
    def _forward_branch(self, features, branch):
        ...
        return {"boxes": torch.cat([v.view(b, 4 * self.reg_max, -1) for v in boxes], dim=-1), "scores": torch.cat([v.view(b, self.nc, -1) for v in scores], dim=-1), "feats": features}

    def forward(self, features):
        self._sync()
        if self.training:
            return {"one2many": self._forward_branch(features, self.one2many), "one2one": self._forward_branch([x.detach() for x in features], self.one2one)}
...
def load_sfr_channel_weights(model, weights):
    model.load(weights)
    source_model = getattr(weights, "model", None)
    target = getattr(model, "model", model)
    source_head, target_head = source_model[-1], target[-1]
    if isinstance(source_head, SFRChannelDetectHead): return {"native_head_items": len(target_head.base_head.state_dict()), "resume": 1}
    result = target_head.base_head.load_state_dict(source_head.state_dict(), strict=True)
    target_head.stride = source_head.stride.detach().clone(); target_head.base_head.stride = target_head.stride
