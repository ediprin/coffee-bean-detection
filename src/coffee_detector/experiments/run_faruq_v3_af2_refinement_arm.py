...
CONFIGS = {
    arm: REPO_ROOT / f"configs/af2_refinement/{arm}_yolo26n.yaml"
...
def run_faruq_v3_af2_refinement_arm(
    arm: str,
    data_root: str | Path,
    ...
    *,
    ...
    if seed != 42:
        raise ValueError("Discovery AF2 refinement dikunci pada seed 42")
    ...
    if audit.get("format") != "coffee_detector.af2_refinement.static_audit.v1":
        raise RuntimeError("Format static audit AF2 refinement tidak valid")
    if audit.get("decision") != "PASS" or not audit.get("training_authorized"):
        raise RuntimeError("Static audit AF2 refinement belum PASS")
    if not audit.get("arm_authorization", {}).get(arm, False):
        raise RuntimeError(f"Static audit tidak mengotorisasi {arm}")
    ...
    refinement = AF2RefinementConfig.from_mapping(payload["refinement"])
    ...
    if not _run_complete(run_dir, epochs):
        ...
        trainer = make_refinement_trainer(refinement, d0_checkpoint=checkpoint)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train one AF2 radial/wavelet refinement arm")
    ...
    run_faruq_v3_af2_refinement_arm(
        args.arm,
        args.data_root,
