"""Matched YOLOv8n prior-study baseline on the frozen J25 development split.
...
CONFIG = REPO_ROOT / "configs/coffee_standard_j25/V8N_MATCHED.yaml"
SAFEAUG_CONFIG = REPO_ROOT / "configs/coffee_standard_j25/SAFEAUG0.yaml"
ARM = "V8N_MATCHED"
PROTOCOL = "coffee-standard-j25-yolov8n-matched-seed42-v1"
...
def run_static_preflight(
    ...
    *,
    ...
    if seed != SEED:
        raise ValueError("Screen V8N-MATCHED pertama dikunci seed 42")
    ...
    payload = {
        "format": "coffee_detector.coffee_standard_j25.yolov8n_matched.static.v1",
        "protocol": PROTOCOL,
...
def run_arm(
    ...
    *,
    ...
    if seed != SEED:
        raise RuntimeError("V8N-MATCHED screening dikunci seed 42")
    ...
    if not dataset["data_format"].endswith("train_siblings.v2"):
        raise RuntimeError("V8N-MATCHED hanya untuk J25 train-siblings v2")
    ...
    contract = {
        "format": "coffee_detector.coffee_standard_j25.yolov8n_matched.arm_contract.v1",
        "protocol": PROTOCOL,
        "arm": ARM,
        "seed": seed,
        ...
        "train": train_args,
        "sampler": "none",
        "comparison_role": "matched nano-scale capacity/family probe; not a CWCF promotion gate",
        "test_images_accessed": False,
    }
    if result_path.is_file():
        old = _json(result_path, "V8N-MATCHED result")
        if old.get("run_contract") != contract:
            raise RuntimeError("Existing V8N-MATCHED result berbeda kontrak")
        return old
    ...
    if not _run_complete(run_dir, int(train_args["epochs"])) or not best.is_file():
        raise RuntimeError("V8N-MATCHED belum selesai secara valid")
    ...
    result = {
        "format": "coffee_detector.coffee_standard_j25.yolov8n_matched.arm_result.v1",
        "protocol": PROTOCOL,
        "arm": ARM,
        "seed": seed,
...
def _target_ap(payload: dict, label: str) -> float:
...
def build_comparison(
    ...
    v8n = _json(v8n_result, "V8N-MATCHED")
    ...
    values = {
        "V8N_MATCHED": {metric: float(v8n["metrics"][metric]) for metric in METRICS},
    ...
    target_values = {
        "V8N_MATCHED": _target_ap(v8n, "V8N_MATCHED"),
        "V8S_MATCHED": _target_ap(v8s, "V8S_MATCHED"),
        "SAFEAUG0": _target_ap(safeaug, "SAFEAUG0"),
        "CWCF1": _target_ap(cwcf, "CWCF1"),
    }
    payload = {
        "format": "coffee_detector.coffee_standard_j25.yolov8n_matched.comparison.v1",
        "protocol": PROTOCOL,
        "seed": SEED,
        "values": values,
        "target_class": TARGET_CLASS,
        ...
        "v8n_minus_safeaug0": {
            metric: values["V8N_MATCHED"][metric] - values["SAFEAUG0"][metric]
            for metric in METRICS
        },
        "v8n_minus_cwcf1": {
            metric: values["V8N_MATCHED"][metric] - values["CWCF1"][metric]
            for metric in METRICS
        },
        "v8s_minus_v8n": {
            metric: values["V8S_MATCHED"][metric] - values["V8N_MATCHED"][metric]
            for metric in METRICS
        },
        "decision": "CAPACITY_FAMILY_PROBE_COMPLETE",
        "claim_boundary": (
            "This is a single-seed descriptive probe. V8N vs V8S informs the size/capacity "
            "gap within YOLOv8; V8N vs YOLO26 controls informs family differences at nano scale. "
            "No locked-test or multi-seed claim is authorized."
        ),
        "test_opened": False,
    }
    destination = Path(output).expanduser().resolve()
