...
from coffee_detector.af2_complement import AF2ComplementConfig, make_af2_complement_trainer
...
def run_faruq_v3_af2_complement_arm(
    arm: str,
    data_root: str | Path,
    ...
    *,
    ...
    complement = AF2ComplementConfig.from_mapping(payload["complement"])
    ...
    if not _complete(run_dir, epochs):
        ...
        trainer = make_af2_complement_trainer(
            afab, complement, initial_checkpoint=checkpoint
        )
    ...
    result = {
        "format": "coffee_detector.af2_complement.arm_result.v1",
        "arm": arm,
        "seed": seed,
        "mode": complement.mode,
        "metrics": metrics,
        "checkpoint": str(best),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train one AF2 complementary arm")
    ...
    run_faruq_v3_af2_complement_arm(
        args.arm,
        args.data_root,
