...
from coffee_detector.af2cal import make_af2cal_trainer
...
def run_faruq_v3_af2cal_arm(
    ...
    *,
    ...
    if audit.get("decision") != "PASS" or not audit.get("training_authorized"):
        raise RuntimeError("Static audit AF2-CAL belum PASS")
    ...
    if payload.get("code") != arm:
        raise RuntimeError("Kode config AF2-CAL tidak konsisten")
    ...
    if not _complete(run_dir, epochs):
        ...
        trainer = (
            make_afab_trainer(afab, d0_checkpoint=checkpoint)
            if arm == "AF2FT30"
            else make_af2cal_trainer(afab, initial_checkpoint=checkpoint)
        )
    ...
    result = {
        "format": "coffee_detector.af2cal.arm_result.v1",
        "arm": arm,
        "seed": seed,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train one AF2 calibration arm")
