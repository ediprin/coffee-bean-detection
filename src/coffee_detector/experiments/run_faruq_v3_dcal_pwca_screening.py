from __future__ import annotations

import argparse
import hashlib
import json
from pathlib import Path

import yaml

from coffee_detector.audit_dataset import audit_dataset
from coffee_detector.dcal_pwca import DCALPWCAConfig, make_dcal_pwca_trainer
from coffee_detector.evaluate import evaluate
from coffee_detector.experiments.run_faruq_v3_baseline import load_faruq_grouped_summary

REPO_ROOT = Path(__file__).resolve().parents[3]
CONFIGS = {
    "SA0": REPO_ROOT / "configs/dcal_pwca/SA0_p5_self_attention_control.yaml",
    "PW1": REPO_ROOT / "configs/dcal_pwca/PW1_p5_pairwise_cross_attention.yaml",
}
METRICS = ("macro_map50_95", "bottom3_class_map50_95", "worst_class_map50_95")


def _sha256(path: str | Path) -> str:
    digest = hashlib.sha256()
    with Path(path).expanduser().resolve().open("rb") as handle:
        for block in iter(lambda: handle.read(1024 * 1024), b""):
            digest.update(block)
    return digest.hexdigest()


def _load_json(path: str | Path, label: str) -> dict:
    source = Path(path).expanduser().resolve()
    if not source.is_file():
        raise FileNotFoundError(f"{label} tidak ditemukan: {source}")
    return json.loads(source.read_text(encoding="utf-8"))


def _metrics(payload: dict) -> dict[str, float]:
    source = payload.get("metrics", payload)
    return {name: float(source[name]) for name in METRICS}


def _delta(a: dict[str, float], b: dict[str, float]) -> dict[str, float]:
    return {name: a[name] - b[name] for name in METRICS}


def _decision(candidate: dict[str, float], d0ft: dict[str, float]) -> dict:
    delta = _delta(candidate, d0ft)
    safeguards = {
        "macro_drop_no_more_than_1_point": delta["macro_map50_95"] >= -0.010,
        "bottom3_drop_no_more_than_2_points": delta["bottom3_class_map50_95"] >= -0.020,
        "worst_drop_no_more_than_2_points": delta["worst_class_map50_95"] >= -0.020,
    }
    signal = (
        delta["macro_map50_95"] >= 0.002
        or delta["bottom3_class_map50_95"] >= 0.005
        or delta["worst_class_map50_95"] >= 0.005
    )
    return {
        "delta_vs_D0FT": delta,
        "criteria": {**safeguards, "has_discovery_signal": signal},
        "decision": "RETAIN" if signal and all(safeguards.values()) else "REJECT",
    }


def _train_arm(
    arm: str,
    data_root: Path,
    output_root: Path,
    d0_checkpoint: Path,
    *,
    seed: int,
    device: str | None,
) -> tuple[Path, bool, dict]:
    payload = yaml.safe_load(CONFIGS[arm].read_text(encoding="utf-8")) or {}
    config = DCALPWCAConfig.from_mapping(payload["dcal_pwca"])
    from ultralytics import YOLO

    run_dir = output_root / f"{arm}_seed{seed}"
    best, last = run_dir / "weights/best.pt", run_dir / "weights/last.pt"
    trainer = make_dcal_pwca_trainer(config, d0_checkpoint=d0_checkpoint)
    executed = False
    if not best.is_file():
        if last.is_file():
            model = YOLO(str(last))
            args = {"resume": True}
            if device is not None:
                args["device"] = device
            model.train(trainer=trainer, **args)
        else:
            model = YOLO(str(REPO_ROOT / payload["model"]))
            args = dict(payload["train"])
            args.update(
                {
                    "data": str(data_root / "data.yaml"),
                    "project": str(output_root),
                    "name": f"{arm}_seed{seed}",
                    "exist_ok": True,
                    "seed": seed,
                    "deterministic": True,
                    "plots": True,
                    "verbose": True,
                }
            )
            if device is not None:
                args["device"] = device
            model.train(trainer=trainer, **args)
        executed = True
    if not best.is_file():
        raise FileNotFoundError(best)
    return best, executed, config.to_dict()


def run_screening(
    data_root: str | Path,
    grouped_summary: str | Path,
    control_summary: str | Path,
    d0_checkpoint: str | Path,
    output_root: str | Path,
    *,
    seed: int = 42,
    device: str | None = None,
    authorize_training: bool = False,
) -> dict:
    if seed != 42:
        raise ValueError("DCAL-PWCA breadth discovery dikunci seed 42")
    if not authorize_training:
        raise RuntimeError("DCAL-PWCA screening belum diotorisasi")
    data_root = Path(data_root).expanduser().resolve()
    output_root = Path(output_root).expanduser().resolve()
    d0_checkpoint = Path(d0_checkpoint).expanduser().resolve()
    load_faruq_grouped_summary(grouped_summary, data_root)
    if (data_root / "test").exists():
        raise RuntimeError("Development root tidak boleh mengekspos split test")

    control = _load_json(control_summary, "D0FT/ACMC1 control summary")
    if control.get("test_images_accessed") is not False or control.get("test_opened") is not False:
        raise RuntimeError("Control summary tidak membuktikan test lock")
    if control.get("d0_checkpoint_sha256") != _sha256(d0_checkpoint):
        raise RuntimeError("Checkpoint D0 berbeda dari control summary")
    controls = {name: _metrics(control["results"][name]) for name in ("D0", "D0FT", "ACMC1")}

    reports = output_root / "val_reports"
    reports.mkdir(parents=True, exist_ok=True)
    audit = audit_dataset(data_root, reports / "dataset_audit.json", near_threshold=-1)
    if not audit["safe_for_training"]:
        raise RuntimeError("Audit dataset gagal")

    candidates, decisions, configs, trained = {}, {}, {}, {}
    for arm in ("SA0", "PW1"):
        best, executed, config = _train_arm(
            arm, data_root, output_root, d0_checkpoint, seed=seed, device=device
        )
        report = evaluate(
            best,
            data_root,
            reports / f"{arm}_seed{seed}_val.json",
            split="val",
            device=device,
        )
        metrics = _metrics(report)
        current = _decision(metrics, controls["D0FT"])
        current["delta_vs_ACMC1"] = _delta(metrics, controls["ACMC1"])
        candidates[arm] = metrics
        decisions[arm] = current
        configs[arm] = config
        trained[arm] = executed

    attribution = {
        "PW1_minus_SA0": _delta(candidates["PW1"], candidates["SA0"]),
        "interpretation": (
            "PW1-SA0 isolates the paired-distractor key/value effect because both arms share "
            "the same P5 attention architecture, D0 initialization, schedule, and seed."
        ),
    }
    result = {
        "protocol": "faruq-v3-dcal-pwca-breadth-screening-v1",
        "stage": "breadth_discovery",
        "seed": seed,
        "evaluation_split": "val",
        "test_images_accessed": False,
        "test_opened": False,
        "d0_checkpoint_sha256": _sha256(d0_checkpoint),
        "paper_mechanism": "DCAL_PWCA_Eq4_training_only_pairwise_cross_attention",
        "paper_facts_retained": {
            "target_image_queries": True,
            "pair_key_value_concatenation": True,
            "paired_image_as_distractor": True,
            "same_classification_target_as_main_branch": True,
            "pwca_removed_at_inference": True,
        },
        "transfer_choices": {
            "feature_level": "P5 only",
            "blocks": 1,
            "hidden_dim": 64,
            "num_heads": 4,
            "pairing": "random cyclic in-batch pairing; batch=1 falls back to self-attention",
            "identity_start": "zero-initialized P5 residual classification correction",
            "one2one_branch": "native and unregularized",
            "glca": "not included; YOLO26 lacks the paper attention-rollout source needed for faithful GLCA",
        },
        "controls": controls,
        "candidate": candidates,
        "decisions": decisions,
        "attribution": attribution,
        "configs": configs,
        "training_executed_this_call": trained,
    }
    summary = reports / "dcal_pwca_seed42_screening.json"
    summary.write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
    result["summary"] = str(summary)
    return result


def main() -> None:
    parser = argparse.ArgumentParser(description="Faruq-v3 DCAL PWCA breadth screening")
    parser.add_argument("--data-root", required=True)
    parser.add_argument("--grouped-summary", required=True)
    parser.add_argument("--control-summary", required=True)
    parser.add_argument("--d0-checkpoint", required=True)
    parser.add_argument("--output-root", required=True)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--device")
    parser.add_argument("--authorize-training", action="store_true")
    args = parser.parse_args()
    result = run_screening(
        args.data_root,
        args.grouped_summary,
        args.control_summary,
        args.d0_checkpoint,
        args.output_root,
        seed=args.seed,
        device=args.device,
        authorize_training=args.authorize_training,
    )
    print(json.dumps(result, indent=2, ensure_ascii=False))


if __name__ == "__main__":
    main()
