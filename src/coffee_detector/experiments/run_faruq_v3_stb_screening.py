...
CONFIG = REPO_ROOT / "configs/stb/STB1_yolo26n_swin_classification.yaml"
...
def run_screening(
    ...
    *,
    ...
    if seed != 42:
        raise ValueError("STB1 discovery dikunci untuk seed 42")
    if not authorize_training:
        raise RuntimeError("STB1 training belum diotorisasi")
    ...
    run_dir = output_root / f"STB1_seed{seed}"
    best, last = run_dir / "weights/best.pt", run_dir / "weights/last.pt"
    trainer = make_stb_trainer(config, d0_checkpoint=d0_checkpoint)
    ...
    report_path = reports / f"STB1_seed{seed}_val.json"
    ...
    result = {
        "protocol": "faruq-v3-zhao-stb-discovery-v1",
        ...
        "design_boundary": (
            "STB1 transfers Zhao et al.'s STB order W-MSA->MLP->SW-MSA->MLP with the paper's "
            "window size 4 and four heads to each YOLO26 P3/P4/P5 classification input. Native box "
            "branches use untouched features. This is not a literal S2A-Net reproduction because YOLO26 "
            "does not provide the paper's ACL-aligned coarse-box feature; a zero-initialized scalar gate "
            "is additionally used only as an identity-start safeguard and is not part of the source paper."
        ),
        "controls": controls,
        "candidate": {"STB1": candidate},
        "delta_vs_D0FT": delta,
    ...
    summary = reports / "stb_seed42_screening.json"
...
def main() -> None:
    p = argparse.ArgumentParser(description="Faruq-v3 Zhao STB discovery")
    ...
    print(json.dumps(run_screening(a.data_root, a.grouped_summary, a.control_summary, a.d0_checkpoint,
                                   ...
                                   authorize_training=a.authorize_training), indent=2,
                     ensure_ascii=False))
