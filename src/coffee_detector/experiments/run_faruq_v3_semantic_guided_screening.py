...
from coffee_detector.semantic_guided import SemanticGuidedConfig, make_semantic_guided_trainer

REPO_ROOT = Path(__file__).resolve().parents[3]
CONFIG = REPO_ROOT / "configs/semantic_guided/SG1_yolo26n_semantic_guided_leaf.yaml"
...
def run_screening(
    ...
    *,
    ...
    if seed != 42:
        raise ValueError("SG1 discovery dikunci untuk seed 42")
    if not authorize_training:
        raise RuntimeError("SG1 training belum diotorisasi")
    ...
    config = SemanticGuidedConfig.from_mapping(payload["semantic_guided"])
    ...
    run_dir = output_root / f"SG1_seed{seed}"
    best, last = run_dir / "weights/best.pt", run_dir / "weights/last.pt"
    training_executed = False
    trainer = make_semantic_guided_trainer(config, d0_checkpoint=d0_checkpoint)
    if not best.is_file():
        if last.is_file():
            model = YOLO(str(last))
            args = {"resume": True}
    ...
    report_path = reports / f"SG1_seed{seed}_val.json"
    ...
    result = {
        "protocol": "faruq-v3-semantic-guided-leaf-discovery-v1",
        ...
        "design_boundary": (
            "SG1 is inspired by DSRDet SSGB Eqs.20-22: ontology semantic logits are predicted "
            "and concatenated back with dense P3/P4/P5 features to produce a residual 21-leaf "
            "classification correction. Unlike literal DSRDet, coffee uses five observable SNI-21 "
            "semantic factors rather than one remote-sensing meta-category, and the guidance starts "
            "from YOLO neck features rather than the paper's exact classification-specialized tensor. "
            "Native box logits remain untouched and native leaf logits are never marginalized."
        ),
        "controls": controls,
        "candidate": {"SG1": candidate},
        "delta_vs_D0FT": delta,
        "delta_vs_ACMC1": {
    ...
    summary = reports / "semantic_guided_seed42_screening.json"
...
def main() -> None:
    p = argparse.ArgumentParser(description="Faruq-v3 semantic-guided leaf discovery")
