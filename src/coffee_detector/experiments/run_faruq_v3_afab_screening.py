...
CONFIGS = {
    "AF1": REPO_ROOT / "configs/afab/AF1_yolo26n_adaptive_highpass.yaml",
    "AF2": REPO_ROOT / "configs/afab/AF2_yolo26n_chaotic_amplitude.yaml",
    "AF12": REPO_ROOT / "configs/afab/AF12_yolo26n_full_afab.yaml",
}
...
def _train_arm(
    ...
    *,
    ...
    config = AFABConfig.from_mapping(payload["afab"])
    from ultralytics import YOLO

    run_dir = output_root / f"{arm}_seed{seed}"
    best, last = run_dir / "weights/best.pt", run_dir / "weights/last.pt"
    trainer = make_afab_trainer(config, d0_checkpoint=d0_checkpoint)
...
def run_screening(
    ...
    *,
    ...
    if seed != 42:
        raise ValueError("AFAB breadth discovery dikunci seed 42")
    if not authorize_training:
        raise RuntimeError("AFAB screening belum diotorisasi")
    ...
    result = {
        ...
        "paper_settings_retained": {
            ...
            "radius_ratio": 0.05,
            "gamma": 0.10,
            "paper_tested_overlap_used_for_breadth": 0.50,
        },
        "transfer_choices": {
            "overlap_note": (
                "LFDet studies overlap 0.50 and 0.75; 0.50 is frozen for breadth-search tractability, "
                "not claimed as the paper default. Any retained AFAB arm must later confirm overlap 0.75."
            ),
            "rgb_frequency_processing": "independent per channel",
            "angular_discretization": "360 bins using floor(theta_degrees), because paper gives theta in [0,360) but no implementation bin count",
            "overlap_reconstruction": "fold with overlap averaging",
            "fft_precision": "float32 internal, restore input dtype",
        },
        "inference_role": "AFAB is active in both training and inference; it is not a train-only augmentation.",
        "controls": controls,
    ...
    summary = reports / "lfdet_afab_seed42_screening.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Faruq-v3 LFDet AFAB breadth screening")
