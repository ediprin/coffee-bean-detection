...
from coffee_detector.af2_spds import AF2SPDSConfig, make_af2_spds_trainer
from coffee_detector.af2_spds.audit import sha256
...
def run_faruq_v3_af2_spds_refinement_arm(
    arm: str,
    data_root: str | Path,
    ...
    *,
    ...
    if arm not in REFINEMENT_ARMS:
        raise ValueError(f"Arm harus salah satu {REFINEMENT_ARMS}")
    if seed != 42:
        raise ValueError("Refinement dikunci pada seed 42")
    ...
    if audit.get("format") != "coffee_detector.af2_spds_refinement.static_audit.v1":
        raise RuntimeError("Static audit refinement tidak sesuai schema")
    if audit.get("decision") != "PASS" or not audit.get("training_authorized"):
        raise RuntimeError("Static audit refinement belum PASS")
    if audit.get("checkpoint_sha256") != sha256(checkpoint):
        raise RuntimeError("Checkpoint AF2 berbeda dari static audit")
    if audit.get("test_access_authorized") is not False:
        raise RuntimeError("Test lock tidak aktif")

    config_path = CONFIGS[arm]
    payload = yaml.safe_load(config_path.read_text(encoding="utf-8")) or {}
    if payload.get("code") != arm:
        raise RuntimeError("Kode config refinement tidak konsisten")
    afab = AFABConfig.from_mapping(payload["afab"])
    spds = AF2SPDSConfig.from_mapping(payload["spds"])
    ...
    if not _complete(run_dir, epochs):
        ...
        trainer = make_af2_spds_trainer(afab, spds, initial_checkpoint=checkpoint)
    ...
    result = {
        "format": "coffee_detector.af2_spds_refinement.arm_result.v1",
        "arm": arm,
        "seed": seed,
        "target": spds.target,
        "auxiliary_schedule": spds.auxiliary_schedule,
        "metrics": metrics,
        "checkpoint": str(best),
        "initial_af2_checkpoint_sha256": sha256(checkpoint),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train one AF2-SPDS refinement arm")
    parser.add_argument("--arm", choices=REFINEMENT_ARMS, required=True)
    ...
    run_faruq_v3_af2_spds_refinement_arm(
        args.arm,
        args.data_root,
