...
from coffee_detector.fbnr import FBNRConfig, make_fbnr_trainer
...
def _load_config(path: Path) -> dict:
    ...
    required = {"code", "model", "fbnr", "train"}
    if not required <= set(payload):
        raise ValueError(f"Config FBNR tidak lengkap: {path}")
    FBNRConfig.from_mapping(payload["fbnr"])
    return payload
...
def _gate(candidate: dict[str, float], d0ft: dict[str, float]) -> dict:
...
def run_screening(
    ...
    *,
    ...
    if not authorize_training:
        raise RuntimeError("FBNR screening belum diotorisasi")
