"""Hybrid energy+directional Haar screen for J25 YOLOv8n CWCF.

HYB1 retains the complete four-channel CWCF1 cue and appends the six signed
LH/HL/HH bands from DIR1. Everything else is matched to V8N_CWCF1.
"""
...
CONFIG = REPO_ROOT / "configs/coffee_standard_j25/V8N_CWCF_HYB1.yaml"
...
ARM = "V8N_CWCF_HYB1"
PROTOCOL = "coffee-standard-j25-yolov8n-cwcf-hyb1-seed42-v1"
...
def _cwcf_without_representation(mapping: dict) -> dict:
    normalized = CWCFConfig.from_mapping(mapping).to_dict()
    return {
        key: value
...
def _build_hybrid_candidate(
    ...
    *,
    ...
    """Build HYB1 while preserving CWCF1 auxiliary-head initialization."""
...
def run_static_preflight(
    ...
    *,
    ...
    if seed != SEED:
        raise ValueError("Screen V8N_CWCF_HYB1 pertama dikunci seed 42")
    ...
    candidate, cwcf1_reference = _build_hybrid_candidate(
    ...
    energy_cue = chromatic_wavelet_cue(
        probe, clip=frozen.cue_clip, detail_mode="energy"
    )
    directional_cue = chromatic_wavelet_cue(
        probe, clip=frozen.cue_clip, detail_mode="directional"
    )
    hybrid_cue = chromatic_wavelet_cue(
        probe, clip=frozen.cue_clip, detail_mode="hybrid"
    )
    ...
    fresh_candidate, _ = _build_hybrid_candidate(
        source, seed, frozen, baseline_frozen
    )
    adapter_zero = all(
        torch.count_nonzero(adapter.affine.weight).item() == 0
        and torch.count_nonzero(adapter.affine.bias).item() == 0
        for adapter in fresh_candidate.model[-1].adapters
    )

    # Channel 4 is LH1 in HYB1, i.e. an appended directional-only cue.
    with torch.no_grad():
        candidate_head.adapters[0].affine.weight[0, 4, 0, 0] = 0.25
    ...
    gates = {
        ...
        "candidate_hybrid_mode_ten_channels":
            frozen.wavelet_detail_mode == "hybrid"
            and frozen.cue_channels == 10,
        "energy_cue_shape_exact": tuple(energy_cue.shape) == (1, 4, 64, 64),
        "directional_cue_shape_exact":
            tuple(directional_cue.shape) == (1, 8, 64, 64),
        "hybrid_cue_shape_exact": tuple(hybrid_cue.shape) == (1, 10, 64, 64),
        "hybrid_preserves_complete_cwcf1_cue":
            torch.equal(hybrid_cue[:, :4], energy_cue),
        "hybrid_preserves_directional_chroma":
            torch.equal(hybrid_cue[:, :2], directional_cue[:, :2]),
        "hybrid_appended_directional_bands_exact":
            torch.equal(hybrid_cue[:, 4:], directional_cue[:, 2:]),
        ...
        "hybrid_adapters_zero_initialized": bool(adapter_zero),
        ...
        "active_hybrid_directional_conditioning_preserves_boxes":
            bool(active_boxes_exact),
        "active_hybrid_directional_conditioning_changes_scores":
            bool(active_scores_change),
    ...
    payload = {
        "format": "coffee_detector.coffee_standard_j25.yolov8n_cwcf_hyb1.static.v1",
        "protocol": PROTOCOL,
...
def _trainer(
    *,
    ...
    class J25V8NCWCFHybridTrainer(DetectionTrainer):
        def get_model(self, cfg=None, weights=None, verbose=True):
            ...
            model, _ = _build_hybrid_candidate(
                source,
                seed,
                config,
                baseline_config,
    ...
    return J25V8NCWCFHybridTrainer
...
def run_arm(
    ...
    *,
    ...
    if seed != SEED:
        raise RuntimeError("V8N_CWCF_HYB1 screening dikunci seed 42")
    ...
    if not dataset["data_format"].endswith("train_siblings.v2"):
        raise RuntimeError("V8N_CWCF_HYB1 hanya untuk J25 train-siblings v2")
    ...
    contract = {
        "format": "coffee_detector.coffee_standard_j25.yolov8n_cwcf_hyb1.arm_contract.v1",
        "protocol": PROTOCOL,
        "arm": ARM,
        "seed": seed,
        ...
        "sampler": "none",
        "comparison_change":
            "retain energy(D1,D2)+append signed directional(LH1,HL1,HH1,LH2,HL2,HH2)",
        "test_images_accessed": False,
    }

    if result_path.is_file():
        old = _json(result_path, "V8N_CWCF_HYB1 result")
        if old.get("run_contract") != contract:
            raise RuntimeError("Existing V8N_CWCF_HYB1 result berbeda kontrak")
        return old
    ...
    if not _run_complete(run_dir, int(train_args["epochs"])) or not best.is_file():
        raise RuntimeError("V8N_CWCF_HYB1 belum selesai secara valid")
    ...
    result = {
        "format": "coffee_detector.coffee_standard_j25.yolov8n_cwcf_hyb1.arm_result.v1",
        "protocol": PROTOCOL,
        "arm": ARM,
        "seed": seed,
...
def build_decision(
    ...
    candidate = _json(candidate_result, "V8N_CWCF_HYB1")
    ...
    if candidate.get("protocol") != PROTOCOL:
        raise RuntimeError("HYB1 candidate protocol salah")
    for label, payload in (
        ("V8N_CWCF1", baseline),
        ("V8N_CWCF_HYB1", candidate),
    ):
    ...
    delta = {
        metric: values["V8N_CWCF_HYB1"][metric] - values["V8N_CWCF1"][metric]
        for metric in METRICS
    }
    target_values = {
        "V8N_CWCF1": float(baseline["target_class_map50_95"]),
        "V8N_CWCF_HYB1": float(candidate["target_class_map50_95"]),
    }
    target_delta = target_values["V8N_CWCF_HYB1"] - target_values["V8N_CWCF1"]

    macro_pass = delta["macro_map50_95"] > 0.0
    tail_pass = delta["bottom3_class_map50_95"] > 0.0
    if macro_pass and tail_pass:
        decision = "PASS_HYB1_SCREEN"
        next_step = "RETAIN_HYBRID_REPRESENTATION"
    elif any(value > 0.0 for value in delta.values()) and any(
        value < 0.0 for value in delta.values()
    ):
        decision = "PARETO_TRADEOFF_HYB1"
        next_step = "REVIEW_HYBRID_REPRESENTATION"
    else:
        decision = "FAIL_HYB1_SCREEN"
        next_step = "RETAIN_V8N_CWCF1"

    payload = {
        "format": "coffee_detector.coffee_standard_j25.yolov8n_cwcf_hyb1.seed42_decision.v1",
        "protocol": PROTOCOL,
        "seed": SEED,
        "values": values,
        "v8n_cwcf_hyb1_minus_cwcf1": delta,
        "target_class": TARGET_CLASS,
        ...
        "next_step": next_step,
        "claim_boundary": (
            "single-seed matched representation screen; HYB1 retains the full "
            "CWCF1 energy cue and appends signed directional Haar bands"
        ),
        "test_opened": False,
    }
    destination = Path(output).expanduser().resolve()
...
def main() -> None:
    ...
    if args.decision_output:
        if args.cwcf1_result is None or args.candidate_result is None:
            parser.error("Decision mode requires CWCF1 and HYB1 results")
