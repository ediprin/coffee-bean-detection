"""Replace CWCF1 attribute BCE with frozen Asymmetric Loss on YOLOv8n.

Everything except the attribute-supervision loss is matched to the completed
V8N_CWCF1 seed-42 arm. Locked test remains closed.
"""
...
CONFIG = REPO_ROOT / "configs/coffee_standard_j25/V8N_CWCF_ASL1.yaml"
...
ARM = "V8N_CWCF_ASL1"
PROTOCOL = "coffee-standard-j25-yolov8n-cwcf-asl1-seed42-v1"
...
ASL_KEYS = {
    ...
    "asl_gamma_neg",
    "asl_clip",
    "asl_detach_focal_weight",
}


def _cwcf_mechanism_mapping(mapping: dict) -> dict:
    """CWCF architecture/supervision contract excluding ASL-only switches."""

    return {key: value for key, value in mapping.items() if key not in ASL_KEYS}
...
def run_static_preflight(
    ...
    *,
    ...
    if seed != SEED:
        raise ValueError("Screen V8N_CWCF_ASL1 pertama dikunci seed 42")

    checkpoint = Path(pretrained_checkpoint).expanduser().resolve()
    if not checkpoint.is_file():
        raise FileNotFoundError(checkpoint)
    baseline_result = _json(matched_baseline_result, "V8N_CWCF1 result")
    ...
    cwcf1_state = cwcf1_reference.state_dict()
    cwcf1_state_exact = (
        list(candidate_state) == list(cwcf1_state)
        and all(
            torch.equal(candidate_state[key].cpu(), cwcf1_state[key].cpu())
    ...
    loss = attribute_asymmetric_loss(
        logits,
        matrix[[7, 8, 12]],
        gamma_pos=frozen.asl_gamma_pos,
        gamma_neg=frozen.asl_gamma_neg,
        clip=frozen.asl_clip,
        detach_focal_weight=frozen.asl_detach_focal_weight,
    ).mean()
    loss.backward()
    ...
    gates = {
        ...
        "same_cwcf_mechanism_as_cwcf1":
            _cwcf_mechanism_mapping(config.get("cwcf", {}))
            == _cwcf_mechanism_mapping(baseline_cfg.get("cwcf", {})),
        "cwcf1_baseline_matches_original_cwcf1":
            baseline_cfg.get("cwcf") == original_cwcf.get("cwcf"),
        "asl_is_only_loss_change":
            frozen.attribute_loss == "asl"
            and baseline_frozen.attribute_loss == "bce"
            and frozen.asl_gamma_pos == 0.0
            and frozen.asl_gamma_neg == 4.0
            and frozen.asl_clip == 0.05
            and frozen.asl_detach_focal_weight is True,
        "candidate_model_state_bitwise_equals_cwcf1": bool(cwcf1_state_exact),
        ...
        "asl_attribute_loss_finite": bool(torch.isfinite(loss)),
        "asl_attribute_gradients_finite_nonzero": bool(
    ...
    payload = {
        "format": "coffee_detector.coffee_standard_j25.yolov8n_cwcf_asl1.static.v1",
        "protocol": PROTOCOL,
        ...
        "cwcf1_config_sha256": _sha256(BASELINE_CONFIG),
...
def run_arm(
    ...
    *,
    ...
    if seed != SEED:
        raise RuntimeError("V8N_CWCF_ASL1 screening dikunci seed 42")
    ...
    if not dataset["data_format"].endswith("train_siblings.v2"):
        raise RuntimeError("V8N_CWCF_ASL1 hanya untuk J25 train-siblings v2")

    baseline = _json(matched_baseline_result, "V8N_CWCF1 result")
    checkpoint = Path(pretrained_checkpoint).expanduser().resolve()
    ...
    contract = {
        "format": "coffee_detector.coffee_standard_j25.yolov8n_cwcf_asl1.arm_contract.v1",
        "protocol": PROTOCOL,
        "arm": ARM,
        "seed": seed,
    ...
    if not _run_complete(run_dir, int(train_args["epochs"])) or not best.is_file():
        raise RuntimeError("V8N_CWCF_ASL1 belum selesai secara valid")
    ...
    result = {
        "format": "coffee_detector.coffee_standard_j25.yolov8n_cwcf_asl1.arm_result.v1",
        "protocol": PROTOCOL,
        "arm": ARM,
        "seed": seed,
...
def build_decision(
    ...
    candidate = _json(candidate_result, "V8N_CWCF_ASL1")
    ...
    if candidate.get("protocol") != PROTOCOL:
        raise RuntimeError("ASL candidate protocol salah")
    for label, payload in (
        ("V8N_CWCF1", baseline),
        ("V8N_CWCF_ASL1", candidate),
    ):
    ...
    delta = {
        metric: values["V8N_CWCF_ASL1"][metric] - values["V8N_CWCF1"][metric]
        for metric in METRICS
    }
    target_values = {
        "V8N_CWCF1": float(baseline["target_class_map50_95"]),
        "V8N_CWCF_ASL1": float(candidate["target_class_map50_95"]),
    }
    target_delta = target_values["V8N_CWCF_ASL1"] - target_values["V8N_CWCF1"]

    macro_pass = delta["macro_map50_95"] > 0.0
    tail_pass = delta["bottom3_class_map50_95"] > 0.0
    if macro_pass and tail_pass:
        decision = "PASS_ASL_SCREEN"
        next_step = "RETAIN_ASL_AND_CONTINUE_CWCF_IMPROVEMENT"
    elif any(value > 0.0 for value in delta.values()) and any(
        value < 0.0 for value in delta.values()
    ):
        decision = "PARETO_TRADEOFF_ASL"
        next_step = "REVIEW_ASL_BEFORE_NEXT_CHANGE"
    else:
        decision = "FAIL_ASL_SCREEN"
        next_step = "RETAIN_V8N_CWCF1"

    payload = {
        "format": "coffee_detector.coffee_standard_j25.yolov8n_cwcf_asl1.seed42_decision.v1",
        "protocol": PROTOCOL,
        "seed": SEED,
        "values": values,
        "v8n_cwcf_asl1_minus_cwcf1": delta,
        "target_class": TARGET_CLASS,
        ...
        "next_step": next_step,
        "claim_boundary": (
            "single-seed matched ASL screen; only the CWCF attribute-supervision "
            "loss changes from BCE to frozen ASL"
        ),
        "test_opened": False,
    }
    destination = Path(output).expanduser().resolve()
...
def main() -> None:
    ...
    if args.decision_output:
        if args.matched_baseline_result is None or args.candidate_result is None:
            parser.error("Decision mode requires CWCF1 baseline and ASL candidate results")
