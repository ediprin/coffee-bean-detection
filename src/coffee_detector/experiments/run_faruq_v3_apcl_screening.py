"""Validation-only broad-search screening for training-only APCL on YOLO26."""
...
from coffee_detector.apcl import make_apcl_trainer


REPO_ROOT = Path(__file__).resolve().parents[3]
CONFIG = REPO_ROOT / "configs/apcl/APCL1_yolo26n_training_only.yaml"
...
def _load_config() -> dict:
    ...
    if payload.get("variant") != "apcl":
        raise RuntimeError("Config APCL1 memiliki variant yang salah")
    if not isinstance(payload.get("apcl"), dict):
        raise RuntimeError("Config APCL1 tidak memiliki mapping apcl")
    return payload


def run_faruq_v3_apcl_screening(
    data_root: str | Path,
    ...
    *,
    ...
    if seed != 42:
        raise ValueError("APCL1 discovery screening dikunci untuk seed 42")
    ...
    if not best_checkpoint.is_file():
        ...
        trainer = make_apcl_trainer(config["apcl"], d0_checkpoint=d0_checkpoint)
        if last_checkpoint.is_file():
            ...
            if device is not None:
                train_args["device"] = device
        print(
            f"{'RESUME' if last_checkpoint.is_file() else 'START'} APCL1 | "
            f"training-only prototype contrast | seed={seed}",
            flush=True,
        )
    ...
    if not best_checkpoint.is_file():
        raise FileNotFoundError(f"APCL1 best.pt tidak ditemukan: {best_checkpoint}")

    report = evaluate(
        best_checkpoint,
        data_root,
        reports_root / f"APCL1_seed{seed}_val.json",
        split="val",
        device=device,
    )
    if report["metrics"].get("classes_without_ground_truth", []):
        raise RuntimeError("Validation kehilangan kelas")
    apcl1 = _metrics(report)

    versus_d0ft = {name: apcl1[name] - d0ft[name] for name in METRICS}
    versus_acmc1 = {name: apcl1[name] - acmc1[name] for name in METRICS}
    discovery_signal = (
    ...
    payload = {
        "protocol": "faruq-v3-apcl-prototype-search-v1",
        ...
        "paper_operator": "APCL_Eqs_9_to_13",
        "adaptation_boundary": (
            "dense positive one-to-many assignments provide embeddings; class "
            "prototypes are EMA buffers; APCL is training-only and inference remains native YOLO26"
        ),
        "d0_checkpoint_sha256": checkpoint_hash,
        "results": {"D0FT": d0ft, "ACMC1": acmc1, "APCL1": apcl1},
        "deltas": {"APCL1_vs_D0FT": versus_d0ft, "APCL1_vs_ACMC1": versus_acmc1},
        "criteria": {"discovery_signal": discovery_signal, **safeguards},
        "decision": decision,
        "next_action": (
            "KEEP_APCL1_FOR_CANDIDATE_POOL"
            if decision == "RETAIN"
            else "ARCHIVE_APCL1_AND_CONTINUE_BROAD_SEARCH"
        ),
        "training_executed_this_call": training_executed_this_call,
    }
    summary = reports_root / "apcl1_seed42_screening.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Faruq-v3 APCL1 broad-search screening")
    ...
    result = run_faruq_v3_apcl_screening(
        args.data_root,
