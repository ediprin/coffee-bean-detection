...
from coffee_detector.af2_spectral import ARMS, SpectralFrontendConfig, make_spectral_trainer
...
TRAIN_ARMS = ARMS[1:]
CONFIGS = {
    arm: REPO_ROOT / f"configs/af2_spectral/{arm}_yolo26n.yaml"
...
def run_faruq_v3_af2_spectral_arm(
    arm: str,
    data_root: str | Path,
    ...
    *,
    ...
    if seed != 42:
        raise ValueError("Breadth screen dikunci pada seed 42")
    ...
    if audit.get("decision") != "PASS" or not audit.get("training_authorized"):
        raise RuntimeError("Static audit spectral belum PASS")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train one AF2 spectral arm")
