"""Export validation-only GT-aligned object events for diagnostic audits.

No training. The exporter rejects any data root exposing test and uses the same
matching convention as the earlier Faruq-v3 complementarity audits:
class-agnostic confidence-ordered matching at IoU >= 0.50.
"""
...
def export_events(checkpoint, data_root, output, *, model_name: str, seed: int = 42, device: str = "0"):
    # Import custom wrappers before checkpoint deserialization.
    import coffee_detector.fsce_cpe.model  # noqa: F401
    import coffee_detector.circle_cpe  # noqa: F401
